...
$pdo->exec("INSERT INTO dbo.SendMail_Campaigns(branch_id,template_id,name,send_mode,status,created_at,completed_at)
    SELECT TOP 101 $otherBranch,$template,'Limite fixture','individual','completed','2020-01-01','2020-02-01' FROM sys.all_objects");
$limited=PurgeRepository::preview(['branch_id'=>$otherBranch,'kind'=>'campaign','channel'=>'email']);
check(count($limited['rows'])===100 && $limited['more'],'Limitar cada purga a cien lotes');
check(PurgeRepository::execute($limited)['totals']['batches']===100,'Procesar el bloque revisado');
