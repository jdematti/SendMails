...
<section class="card">
    <div class="actions compose-top">
        <a class="btn secondary" href="activity.php?kind=<?= e($composeKind) ?>">Ver envíos</a>
        <label>Borradores compartidos
    ...
    <nav class="compose-steps" aria-label="Preparar envío">
        ...
        <button type="button" id="reviewTop">3. Revisar y programar</button>
    </nav>
    <form id="composeForm">
        <section data-panel="1">
            <div class="form-grid">
                ...
                <?php if (!$isInvoice): ?>
                    ...
                    <div class="field"><label for="plans">Planes</label><select id="plans" name="plans" multiple size="4"><?php foreach ($plans as $plan): ?><option value="<?= e($plan) ?>"><?= e($plan) ?></option><?php endforeach; ?></select><span class="hint">Sin selección: todos los planes.</span></div>
                <?php else: ?>
            ...
            <div class="selection-toolbar">
                ...
                <button type="button" class="btn secondary" id="selectPage">Seleccionar esta página</button>
                <button type="button" class="btn secondary" id="selectAll">Seleccionar todos los resultados</button>
                <button type="button" class="btn secondary" id="clearSelection">Vaciar selección</button>
            </div>
            ...
            <?php if (!$isInvoice): ?><div class="field" data-email-field><label for="manual_emails">Emails adicionales</label><textarea id="manual_emails" name="manual_emails" rows="2" placeholder="Podés enviar solo a emails manuales. Separalos con coma, espacio o salto de línea."></textarea></div><?php endif; ?>
            <div class="actions"><button type="button" data-step="2">Continuar al mensaje</button></div>
        </section>
        <section data-panel="2" hidden>
            ...
            <div class="actions"><button type="button" class="btn secondary" data-step="1">Volver a destinatarios</button><button type="button" id="reviewSend">Revisar envío</button></div>
        </section>
        <section data-panel="3" hidden>
            ...
            <div class="actions"><button type="button" class="btn secondary" data-step="2">Modificar</button><button type="button" id="confirmSend">Confirmar y programar</button></div>
        </section>
    </form>
</section>
